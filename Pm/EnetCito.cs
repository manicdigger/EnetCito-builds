...
sub serialize_command($$$) {
	...
	$self->write_short($buf, $pos, $a->{header}->{reliable_sequence_number});
	{
		...
		elsif (($a->{header}->{command} & 15) == 4) {
			$self->write_int($buf, $pos, $a->{disconnect}->{data});
			last;
		}
		elsif (($a->{header}->{command} & 15) == 5) {
			last;
		}
		elsif (($a->{header}->{command} & 15) == 6) {
			$self->write_short($buf, $pos, $a->{send_reliable}->{data_length});
			last;
		}
		elsif (($a->{header}->{command} & 15) == 7) {
		...
		elsif (($a->{header}->{command} & 15) == 8 || ($a->{header}->{command} & 15) == 12) {
...
sub enet_host_service($$$$) {
	...
	if (defined($event_)) {
		...
		$event_->{peer} = undef;
		$event_->{packet} = undef;
		my $CISWITCH = $self->enet_protocol_dispatch_incoming_commands($host, $event_);
		if ($CISWITCH == 1) {
			return 1;
		}
		elsif ($CISWITCH == -1) {
	...
	do {
		...
		my $CISWITCH = $self->enet_protocol_send_outgoing_commands($host, $event_, 1);
		if ($CISWITCH == 1) {
			return 1;
		}
		elsif ($CISWITCH == -1) {
		...
		my $CISWITCH = $self->enet_protocol_receive_incoming_commands($host, $event_);
		if ($CISWITCH == 1) {
			return 1;
		}
		elsif ($CISWITCH == -1) {
...
sub enet_peer_queue_incoming_command($$$$$) {
	...
	if (($command->{header}->{command} & 15) == 8 || ($command->{header}->{command} & 15) == 6) {
		if ($reliableSequenceNumber == $channel->{incoming_reliable_sequence_number}) {
			return $self->free_packet($fragmentCount, $packet);
		}
	...
	elsif (($command->{header}->{command} & 15) == 7 || ($command->{header}->{command} & 15) == 12) {
	...
	elsif (($command->{header}->{command} & 15) == 9) {
...
sub enet_peer_setup_outgoing_command($$$) {
	...
	if (($outgoingCommand->{command}->{header}->{command} & 15) == 7) {
	...
	elsif (($outgoingCommand->{command}->{header}->{command} & 15) == 9) {
...
sub enet_protocol_handle_incoming_commands($$$) {
	...
	while ($currentDataI->[0] < $host->{received_data_length}) {
		...
		if (defined($peer) && ($command->{header}->{command} & 128) != 0) {
			...
			if ($peer->{state} == 7 || $peer->{state} == 2 || $peer->{state} == 0 || $peer->{state} == 9) {
			}
			elsif ($peer->{state} == 8) {
...
sub enet_protocol_receive_incoming_commands($$$) {
	...
	for (;;) {
		...
		if (defined($host->{intercept})) {
			my $CISWITCH = $host->{intercept}->run($host, $event_);
			if ($CISWITCH == 1) {
				if (defined($event_) && $event_->{type} != ENetEventType::E_N_E_T__E_V_E_N_T__T_Y_P_E__N_O_N_E()) {
					return 1;
				}
		...
		my $CISWITCH = $self->enet_protocol_handle_incoming_commands($host, $event_);
		if ($CISWITCH == 1) {
			return 1;
		}
		elsif ($CISWITCH == -1) {
